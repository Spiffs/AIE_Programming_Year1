...
bool QuickCheck(char player, int dir, int x, int y, const OthelloBoardState& grid)
{
	...
	place = false;
	while (temp.x < 8 || temp.x >= 0 || temp.y < 8 || temp.y >= 0)
	{
		if (temp.y + ycase > 7 || temp.x + xcase > 7 ||
			temp.y + ycase < 0 || temp.x + xcase < 0 ||
			grid[temp.y + ycase][temp.x + xcase] == ' ')
			return false;
		if (grid[temp.y + ycase][temp.x + xcase] == player && place)
			return true;
		if (grid[temp.y + ycase][temp.x + xcase] != player)
			place = true;
		temp = { temp.y + ycase, temp.x + xcase };
	}
...
{
	// straights
	if (QuickCheck(player, 1, x, y, grid))
		return true;
	if (QuickCheck(player, 2, x, y, grid))
		return true;
	if (QuickCheck(player, 3, x, y, grid))
		return true;
	if (QuickCheck(player, 4, x, y, grid))
		return true;

	// diagonals
	if (QuickCheck(player, 5, x, y, grid))
		return true;
	if (QuickCheck(player, 6, x, y, grid))
		return true;
	if (QuickCheck(player, 7, x, y, grid))
		return true;
	if (QuickCheck(player, 8, x, y, grid))
		return true;
